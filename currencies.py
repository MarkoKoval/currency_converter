...
class Ccy:
    ...
    currency_rates = {item["cc"]: (item["txt"], item["rate"]) for item in json.loads(requests.get(
        "https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?date={}&json".format(
    ...
    def get_rate(self, base_currency, secondary_currency):
        ...
        if base_currency == "UAH":
            ...
            return self.currency_rates[secondary_currency][1] / self.currency_rates[base_currency][1]
    ...
    def __add__(self, secondary):
        ...
        if self.currency == secondary.currency:
            return Ccy(self.amount + secondary.amount, self.currency)
                ...
                (self.currency in self.currency_rates and secondary.currency in self.currency_rates) or
                (self.currency == "UAH" and secondary.currency in self.currency_rates) or
                (self.currency in self.currency_rates and secondary.currency == "UAH")):
            return Ccy(self.amount + self.find_rate(secondary.amount, self.currency, secondary.currency), self.currency)
        else:
            raise Exception("Enter correct data")

    def __sub__(self, secondary):
        """ subtract converted secondary currency from base"""
        if self.currency == secondary.currency and self.amount > secondary.amount:
            return Ccy(self.amount - secondary.amount, self.currency)
        elif self.currency != secondary.currency and (
                (self.currency in self.currency_rates and secondary.currency in self.currency_rates) or
                (self.currency == "UAH" and secondary.currency in self.currency_rates) or
                (self.currency in self.currency_rates and secondary.currency == "UAH")):
            converted = self.find_rate(secondary.amount, self.currency, secondary.currency)
